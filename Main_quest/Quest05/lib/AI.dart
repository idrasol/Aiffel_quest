...
import 'DogHeadDetector.dart'; // DogHeadDetector 클래스 import
...
class _AIScreenState extends State<AIScreen> {
  ...
  Future<void> _predictImage() async {
    ...
    try {
      ...
      var uri = Uri.parse('https://5fef-221-145-193-52.ngrok-free.app/predict');
  ...
  Future<void> _detectDogHead() async {
    setState(() {
      _isLoading = true;
      _errorMessage = null;
      _successMessage = null;
    ...
    try {
      final result = await DogHeadDetector.detectDogHead(widget.photo.url);
      ...
      if (result.containsKey('error')) {
        ...
        setState(() {
          _predictedLabel = '개 머리 감지 완료!';
          _predictionScore = '감지된 개 머리: ${result['detected_heads'] ?? '없음'}'; // 예를 들어 감지된 개 머리 수를 표시
          _errorMessage = null;
          _successMessage = '개 머리 감지가 성공적으로 완료되었습니다!';
        });
      }
    } catch (e) {
      setState(() {
